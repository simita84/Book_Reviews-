...
class Dashboard extends Main {
	...
	public function create_book_and_review()
	{
		...
		if($review_details['new_author'])
	   	{
	   	  	...
	   	  	$author_created     =  $this->Author->create_author($author_first_name,
	   	  		                    $author_last_name  );
    	...
    	if($book_created)
    	{
    		$review_created = $this->Review->create_review($review_details,
    						 $author_id,$book_id ,$user_id);
    		if($review_created)
    		{
    			$this->session->set_flashdata("success_message","Review created for the 
    				                        new book");
	...
	public function create_review($book_id,$author_id)
	{
		...
		//$review_details =  array('review' => , );
		$review_created = $this->Review->create_review($this->input->post(),
    						         $author_id,$book_id ,$user_id);
	...
	public function show_user($id)
	{
		...
		$all_books = $this->Book->get_all_books();
		$reviews_created_by_user = 
		            $this->Review->get_reviews_by_user($id);
